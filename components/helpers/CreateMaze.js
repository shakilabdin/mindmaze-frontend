...
const CreateMaze = (gridX = 15, gridY = 18) => {
    ...
    generateWall = (x, y) => {
        ...
        if (gridPoint !== LEFT && leftPoint !== this.getOpposite(LEFT)) {
            Matter.Composite.add(
                ...
                Matter.Bodies.rectangle(
                    ...
                    this.blockHeight / 2,
                    wallThickness,
                    this.blockHeight + wallThickness,
                    opts
                )
            );
        }
        ...
        const translate = Matter.Vector.create(
            x * this.blockWidth,
            y * this.blockHeight
        );
    ...
    carvePathFrom = (x, y, grid) => {
        const directions = [TOP, BOTTOM, RIGHT, LEFT].sort(
            f => 0.5 - GetRandomNumber()
        ); // whichever direction is closest to the random number is first in the list.
